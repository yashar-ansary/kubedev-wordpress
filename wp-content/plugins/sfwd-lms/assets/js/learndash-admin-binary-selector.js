...
function learndash_binary_selector( selector_div ) {
	...
	this.init_pager = function() {
		if ( jQuery( 'ul.learndash-binary-selector-pager', self.selector_div ).length ) {
			...
			jQuery( '.learndash-binary-selector-section ul.learndash-binary-selector-pager a', self.selector_div ).on( 'click', function( e ) {
				e.preventDefault();
...
*/
	this.get_selector_form_element = function( return_raw ) {
		...
		if ( return_raw == false ) {
			...
			self.selected_items = jQuery.map( self.selected_items, function( el ) {
				return parseInt( el );
			} );
