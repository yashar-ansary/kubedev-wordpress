<?php
/**
 * LearnDash Data Upgrades for Course Post Meta
 *
 * @since 3.4.1
...
if ( ( class_exists( 'Learndash_Admin_Data_Upgrades' ) ) && ( ! class_exists( 'Learndash_Admin_Data_Upgrades_Course_Post_Meta' ) ) ) {
	/**
	 * Class to create the Data Upgrade for Course Post Meta.
	 */
	class Learndash_Admin_Data_Upgrades_Course_Post_Meta extends Learndash_Admin_Data_Upgrades {
		...
		public function process_post_meta_items() {
			...
			if ( ! empty( $post_ids_diff ) ) {
				...
				foreach ( $post_ids_diff as $post_id ) {
					...
					$price_type = learndash_get_setting( $post_id, 'course_price_type' );
...
add_action(
	...
	function() {
		Learndash_Admin_Data_Upgrades_Course_Post_Meta::add_instance();
	}
);
