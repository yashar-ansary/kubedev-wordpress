...
class WpProQuiz_Helper_DbUpgrade {
	...
	public function upgrade( $version ) {
		...
		if ( ( defined( 'LEARNDASH_ACTIVATED' ) ) || ( false === $version ) || ( (int) $version > self::WPPROQUIZ_DB_VERSION ) ) {
			$this->install();
			return self::WPPROQUIZ_DB_VERSION;
		}

		$version = (int) $version;

		if ( self::WPPROQUIZ_DB_VERSION === $version ) {
			return self::WPPROQUIZ_DB_VERSION;
		}
		...
		} while ( $version < self::WPPROQUIZ_DB_VERSION );
	...
	public function databaseDelta() {
		...
		dbDelta(
			'
			CREATE TABLE ' . LDLMS_DB::get_table_name( 'quiz_category' ) . ' (
			  category_id int(10) unsigned NOT NULL AUTO_INCREMENT,
			  category_name varchar(200) NOT NULL,
			  PRIMARY KEY  (category_id)
			)  ' . $charset_collate . ';

			CREATE TABLE ' . LDLMS_DB::get_table_name( 'quiz_form' ) . ' (
			...
			CREATE TABLE ' . LDLMS_DB::get_table_name( 'quiz_lock' ) . ' (
			  quiz_id int(11) NOT NULL,
			...
			CREATE TABLE ' . LDLMS_DB::get_table_name( 'quiz_prerequisite' ) . ' (
			  prerequisite_quiz_id int(11) NOT NULL,
			  quiz_id int(11) NOT NULL,
			  PRIMARY KEY  (prerequisite_quiz_id,quiz_id)
			)  ' . $charset_collate . ';

			CREATE TABLE ' . LDLMS_DB::get_table_name( 'quiz_question' ) . ' (
			  id int(11) NOT NULL AUTO_INCREMENT,
			  quiz_id int(11) NOT NULL,
			...
			CREATE TABLE ' . LDLMS_DB::get_table_name( 'quiz_template' ) . ' (
			...
			CREATE TABLE ' . LDLMS_DB::get_table_name( 'quiz_toplist' ) . ' (
		...
		if ( false === $valid_index ) {
			...
			$wpdb->query( $wpdb->prepare( 'DELETE FROM ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_category' ) ) . ' WHERE category_id = %d', 0 ) );

			$wpdb->query( 'ALTER TABLE ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_category' ) ) . ' MODIFY COLUMN category_id int(10) unsigned NOT NULL AUTO_INCREMENT' );
		}

		$valid_index = LDLMS_DB::check_table_primary_index( 'quiz_form' );
		if ( false === $valid_index ) {
			// If the AUTO_INCREMENT attrribute is missing we want to also remove any records where the primary index field is zero.
			$wpdb->query( $wpdb->prepare( 'DELETE FROM ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_form' ) ) . ' WHERE form_id = %d', 0 ) );

			$wpdb->query( 'ALTER TABLE ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_form' ) ) . ' MODIFY COLUMN form_id int(11) NOT NULL AUTO_INCREMENT' );
		}

		$valid_index = LDLMS_DB::check_table_primary_index( 'quiz_master' );
		if ( false === $valid_index ) {
			// If the AUTO_INCREMENT attrribute is missing we want to also remove any records where the primary index field is zero.
			$wpdb->query( $wpdb->prepare( 'DELETE FROM ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_master' ) ) . ' WHERE id = %d', 0 ) );

			$wpdb->query( 'ALTER TABLE ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_master' ) ) . ' MODIFY COLUMN id int(11) NOT NULL AUTO_INCREMENT' );
		}

		$valid_index = LDLMS_DB::check_table_primary_index( 'quiz_question' );
		if ( false === $valid_index ) {
			// If the AUTO_INCREMENT attrribute is missing we want to also remove any records where the primary index field is zero.
			$wpdb->query( $wpdb->prepare( 'DELETE FROM ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_question' ) ) . ' WHERE id = %d', 0 ) );

			$wpdb->query( 'ALTER TABLE ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_question' ) ) . ' MODIFY COLUMN id int(11) NOT NULL AUTO_INCREMENT' );
		}

		$valid_index = LDLMS_DB::check_table_primary_index( 'quiz_statistic_ref' );
		if ( false === $valid_index ) {
			// If the AUTO_INCREMENT attrribute is missing we want to also remove any records where the primary index field is zero.
			$wpdb->query( $wpdb->prepare( 'DELETE FROM ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_statistic_ref' ) ) . ' WHERE statistic_ref_id = %d', 0 ) );

			$wpdb->query( 'ALTER TABLE ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_statistic_ref' ) ) . ' MODIFY COLUMN statistic_ref_id int(10) unsigned NOT NULL AUTO_INCREMENT' );
		}

		$valid_index = LDLMS_DB::check_table_primary_index( 'quiz_template' );
		if ( false === $valid_index ) {
			// If the AUTO_INCREMENT attrribute is missing we want to also remove any records where the primary index field is zero.
			$wpdb->query( $wpdb->prepare( 'DELETE FROM ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_template' ) ) . ' WHERE template_id = %d', 0 ) );

			$wpdb->query( 'ALTER TABLE ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_template' ) ) . ' MODIFY COLUMN template_id int(11) NOT NULL AUTO_INCREMENT' );
		}

		$valid_index = LDLMS_DB::check_table_primary_index( 'quiz_toplist' );
		if ( false === $valid_index ) {
			// If the AUTO_INCREMENT attrribute is missing we want to also remove any records where the primary index field is zero.
			$wpdb->query( $wpdb->prepare( 'DELETE FROM ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_toplist' ) ) . ' WHERE toplist_id = %d', 0 ) );

			$wpdb->query( 'ALTER TABLE ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_toplist' ) ) . ' MODIFY COLUMN toplist_id int(11) NOT NULL AUTO_INCREMENT' );
		}
	}
	...
	private function upgradeDbV3() {
		...
		$this->_wpdb->query(
			'
			ALTER TABLE `' . LDLMS_DB::get_table_name( 'quiz_question' ) . '`
		...
		$this->_wpdb->query(
			'
			CREATE TABLE IF NOT EXISTS `' . LDLMS_DB::get_table_name( 'quiz_lock' ) . '` (
				`quiz_id` int(11) NOT NULL,
				`lock_ip` varchar(100) NOT NULL,
				`lock_date` int(11) NOT NULL,
	...
	private function upgradeFixDbV4() {
		if ( 'wp_' != $this->_wpdb->prefix ) {
			$this->_wpdb->query( 'SELECT * FROM `' . LDLMS_DB::get_table_name( 'quiz_question' ) . '` LIMIT 0,1' );

			$names = $this->_wpdb->get_col_info( 'name' );

			if ( ! in_array( 'tip_enabled', $names ) ) {
				$this->_wpdb->query( 'ALTER TABLE  `' . LDLMS_DB::get_table_name( 'quiz_question' ) . '` ADD `tip_enabled` TINYINT( 1 ) NOT NULL AFTER  `incorrect_count`' );
			}

			if ( ! in_array( 'tip_msg', $names ) ) {
				$this->_wpdb->query( 'ALTER TABLE  `' . LDLMS_DB::get_table_name( 'quiz_question' ) . '` ADD `tip_msg` TEXT NOT NULL AFTER  `tip_enabled`' );
			}

			if ( ! in_array( 'tip_count', $names ) ) {
				$this->_wpdb->query( 'ALTER TABLE  `' . LDLMS_DB::get_table_name( 'quiz_question' ) . '` ADD  `tip_count` INT NOT NULL AFTER `tip_msg`' );
			}
		}
	}
	...
	private function upgradeDbV6() {
		...
		$this->_wpdb->query(
			'
			UPDATE `' . LDLMS_DB::get_table_name( 'quiz_question' ) . '` SET `points` = 1
		'
		);
	...
	private function upgradeDbV7() {
		...
		$this->_wpdb->query(
			'
			ALTER TABLE  `' . LDLMS_DB::get_table_name( 'quiz_lock' ) . '`
		...
		$this->_wpdb->query(
			'
			ALTER TABLE  `' . LDLMS_DB::get_table_name( 'quiz_lock' ) . '` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci
		'
		);

		$this->_wpdb->query(
			'
			ALTER TABLE  `' . LDLMS_DB::get_table_name( 'quiz_master' ) . '` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci
		'
		);

		$this->_wpdb->query(
			'
			ALTER TABLE  `' . LDLMS_DB::get_table_name( 'quiz_question' ) . '` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci
		'
		);
	...
	private function upgradeDbV9() {
		...
		$this->_wpdb->query(
			'
			TRUNCATE `' . LDLMS_DB::get_table_name( 'quiz_lock' ) . '`
		'
		);
		...
		$this->_wpdb->query(
			'
			CREATE TABLE IF NOT EXISTS `' . LDLMS_DB::get_table_name( 'quiz_statistic' ) . '` (
				  `quiz_id` int(11) NOT NULL,
				  `question_id` int(11) NOT NULL,
				  `user_id` bigint(20) unsigned NOT NULL,
				  `correct_count` int(10) unsigned NOT NULL,
		...
		$this->_wpdb->query(
			'
			INSERT INTO `' . LDLMS_DB::get_table_name( 'quiz_statistic' ) . '` (quiz_id, question_id, user_id, correct_count, incorrect_count, hint_count)
				SELECT
					question.quiz_id, id, 0, question.correct_count, question.incorrect_count, tip_count
				FROM
					`' . LDLMS_DB::get_table_name( 'quiz_question' ) . '` as question
				WHERE
	...
	private function upgradeDbV11() {
		...
		$this->_wpdb->query(
			'
			ALTER TABLE `' . LDLMS_DB::get_table_name( 'quiz_statistic' ) . '`
				ADD  `correct_answer_count` INT UNSIGNED NOT NULL
		'
		);

		$this->_wpdb->query( 'UPDATE `' . LDLMS_DB::get_table_name( 'quiz_statistic' ) . '` SET `correct_answer_count` = `correct_count`' );

		$this->_wpdb->query( 'UPDATE `' . LDLMS_DB::get_table_name( 'quiz_question' ) . '` SET `points_answer` = `points`' );
	...
	private function upgradeDbV13() {
		...
		if ( ! in_array( 'hide_answer_message_box', $names ) ) {
			$this->_wpdb->query( 'ALTER TABLE  `' . LDLMS_DB::get_table_name( 'quiz_master' ) . '` ADD  `hide_answer_message_box` TINYINT( 1 ) NOT NULL' );
		}

		if ( ! in_array( 'disabled_answer_mark', $names ) ) {
			$this->_wpdb->query( 'ALTER TABLE  `' . LDLMS_DB::get_table_name( 'quiz_master' ) . '` ADD  `disabled_answer_mark` TINYINT( 1 ) NOT NULL' );
		}

		if ( ! in_array( 'show_max_question', $names ) ) {
			$this->_wpdb->query( 'ALTER TABLE  `' . LDLMS_DB::get_table_name( 'quiz_master' ) . '` ADD  `show_max_question` TINYINT( 1 ) NOT NULL' );
		}

		if ( ! in_array( 'show_max_question_value', $names ) ) {
			$this->_wpdb->query( 'ALTER TABLE  `' . LDLMS_DB::get_table_name( 'quiz_master' ) . '` ADD  `show_max_question_value` INT UNSIGNED NOT NULL' );
		}

		if ( ! in_array( 'show_max_question_percent', $names ) ) {
			$this->_wpdb->query( 'ALTER TABLE  `' . LDLMS_DB::get_table_name( 'quiz_master' ) . '` ADD  `show_max_question_percent` TINYINT( 1 ) NOT NULL' );
		}
	...
	private function upgradeDbV15() {
		...
		$this->_wpdb->query(
			'
			CREATE TABLE IF NOT EXISTS `' . LDLMS_DB::get_table_name( 'quiz_prerequisite' ) . '` (
			 	`prerequisite_quiz_id` int(11) NOT NULL,
			 	`quiz_id` int(11) NOT NULL,
			  	PRIMARY KEY (`prerequisite_quiz_id`,`quiz_id`)
			) DEFAULT CHARSET=utf8;
		...
		$this->_wpdb->query(
			'
			CREATE TABLE IF NOT EXISTS `' . LDLMS_DB::get_table_name( 'quiz_toplist' ) . '` (
				  `toplist_id` int(11) NOT NULL AUTO_INCREMENT,
				  `quiz_id` int(11) NOT NULL,
				  `date` int(10) unsigned NOT NULL,
				  `user_id` bigint(20) unsigned NOT NULL,
		...
		$results = $this->_wpdb->get_results( 'SELECT id, answer_type, answer_json, points_per_answer, points_answer  FROM `' . LDLMS_DB::get_table_name( 'quiz_question' ) . '`', ARRAY_A );
		...
		foreach ( $results as $row ) {
			...
			if ( 'single' == $row['answer_type'] || 'multiple' == $row['answer_type'] ) {
			...
			} elseif ( 'cloze_answer' == $row['answer_type'] ) {
				$x = new WpProQuiz_Model_AnswerTypes();

				$x->setAnswer( $data['answer_cloze']['text'] );

				$newData[] = $x;
			} elseif ( 'matrix_sort_answer' == $row['answer_type'] ) {
			...
			} elseif ( 'free_answer' == $row['answer_type'] ) {
				$x = new WpProQuiz_Model_AnswerTypes();

				$x->setAnswer( $data['free_answer']['correct'] );

				$newData[] = $x;
			} elseif ( 'sort_answer' == $row['answer_type'] ) {
		...
		$this->_wpdb->query(
			'UPDATE ' . LDLMS_DB::get_table_name( 'quiz_question' ) . '
			SET
		...
		$this->_wpdb->query(
			...
			SET
				s.points = ( SELECT q.points_answer FROM ' . LDLMS_DB::get_table_name( 'quiz_question' ) . ' AS q WHERE q.id = s.question_id ) * s.correct_answer_count
			WHERE
	...
	private function upgradeDbV17() {
		...
		$this->_wpdb->query(
			'
			CREATE TABLE IF NOT EXISTS `' . LDLMS_DB::get_table_name( 'quiz_category' ) . '` (
			  `category_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
			  `category_name` varchar(200) NOT NULL,
			  PRIMARY KEY (`category_id`)
			)  DEFAULT CHARSET=utf8;
		...
		$this->_wpdb->update(
			LDLMS_DB::get_table_name( 'quiz_master' ),
			array(
	...
	private function upgradeDbV18() {
		...
		$this->_wpdb->query(
			'
			DELETE s
				FROM ' . LDLMS_DB::get_table_name( 'quiz_statistic' ) . ' AS s
					LEFT JOIN ' . LDLMS_DB::get_table_name( 'quiz_master' ) . ' AS m ON ( s.quiz_id = m.id )
					LEFT JOIN ' . LDLMS_DB::get_table_name( 'quiz_question' ) . ' AS q ON ( s.question_id = q.id )
		...
		$this->_wpdb->query(
			'
			INSERT INTO ' . LDLMS_DB::get_table_name( 'quiz_statistic_ref' ) . '
		...
		$this->_wpdb->query(
			'
			UPDATE ' . LDLMS_DB::get_table_name( 'quiz_statistic' ) . ' AS s
				LEFT JOIN ' . LDLMS_DB::get_table_name( 'quiz_statistic_ref' ) . ' AS sf
		...
		if ( in_array( 'check_answer', $names ) ) {
			$this->_wpdb->query( 'ALTER TABLE  `' . LDLMS_DB::get_table_name( 'quiz_master' ) . '` DROP `check_answer` ' );
		}

		if ( in_array( 'back_button', $names ) ) {
			$this->_wpdb->query( 'ALTER TABLE  `' . LDLMS_DB::get_table_name( 'quiz_master' ) . '` DROP `back_button` ' );
		}

		if ( in_array( 'question_on_single_page', $names ) ) {
			$this->_wpdb->query( 'ALTER TABLE  `' . LDLMS_DB::get_table_name( 'quiz_master' ) . '` DROP `question_on_single_page` ' );
		}
	...
	private function upgradeDbV21() {
		...
		$this->_wpdb->query(
			'
			ALTER TABLE  ' . LDLMS_DB::get_table_name( 'quiz_statistic_ref' ) . '
		...
		$this->_wpdb->query(
			'
			CREATE TABLE IF NOT EXISTS  `' . LDLMS_DB::get_table_name( 'quiz_form' ) . '` (
			  `form_id` int(11) NOT NULL AUTO_INCREMENT,
			  `quiz_id` int(11) NOT NULL,
			  `fieldname` varchar(100) NOT NULL,
			  `type` tinyint(4) NOT NULL,
		...
		$this->_wpdb->query(
			'
			CREATE TABLE IF NOT EXISTS  `' . LDLMS_DB::get_table_name( 'quiz_template' ) . '` (
			  `template_id` int(11) NOT NULL AUTO_INCREMENT,
			  `name` varchar(200) NOT NULL,
			  `type` tinyint(3) unsigned NOT NULL,
			  `data` text NOT NULL,
