...
<div class="wrap wpProQuiz_globalSettings">
	<h2 style="margin-bottom: 10px;">
		...
		echo sprintf( esc_html_x( '%s Options', ': Quiz', 'learndash' ), LearnDash_Custom_Label::get_label( 'quiz' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
		?>
	</h2>
	...
	<div class="wpProQuiz_tab_wrapper" style="padding: 10px 0px;">
		...
		<?php
		// translators: placeholder: Quiz.
		echo sprintf( esc_html_x( '%s Options', 'placeholder: Quiz', 'learndash' ), LearnDash_Custom_Label::get_label( 'quiz' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
		?>
		</a>
						...
						<tr>
							...
							<td>
								...
								if ( ( ! empty( $this->email['from'] ) ) && ( ! is_email( $this->email['from'] ) ) ) {
									?>
									<p class="ld-error error-message"><?php esc_html_e( 'The value entered is not a valid email address', 'learndash' ); ?></p>
									<?php
								}
								?>
								<label>
								...
								<p class="description">
									...
									echo sprintf( wp_kses_post( _x( 'This is the email address of the sender. If not provided the admin email <strong>(%s)</strong> will be used.', 'placeholder: Admin Email', 'learndash' ) ), esc_html( get_option( 'admin_email' ) ) );
									?>
								</p>
							</td>
						</tr>
