<?php
/**
 * LearnDash REST API V2 Groups Users Controller.
 *
 * This Controller class is used to GET/UPDATE/DELETE the association
 * between a Course (sfwd-courses) and Users enrolled into the group.
 *
...
if ( ( ! class_exists( 'LD_REST_Groups_Users_Controller_V2' ) ) && ( class_exists( 'LD_REST_Users_Controller_V2' ) ) ) {

	/**
	 * Class LearnDash REST API V2 Groups Users Controller.
	 *
	 * @since 3.3.0
	...
	class LD_REST_Groups_Users_Controller_V2 extends LD_REST_Users_Controller_V2 {
		...
		public function __construct() {
			...
			$this->rest_sub_base = $this->get_rest_base( 'groups-users' );
		}
			...
			register_rest_route(
				...
				array(
					...
					array(
						...
						'callback'            => array( $this, 'get_groups_users' ),
						'permission_callback' => array( $this, 'get_groups_users_permissions_check' ),
						'args'                => $this->get_collection_params(),
					),
					array(
						'methods'             => WP_REST_Server::EDITABLE,
						'callback'            => array( $this, 'update_groups_users' ),
					...
					array(
						...
						'callback'            => array( $this, 'delete_groups_users' ),
		...
		public function get_groups_users_permissions_check( $request ) {
			...
			if ( learndash_is_group_leader_user() ) {
				...
				$leader_groups = array_map( 'intval', $leader_groups );
				if ( ( ! empty( $leader_groups ) ) && ( in_array( absint( $group_id ), $leader_groups, true ) ) ) {
					return true;
				}
			}
		}
