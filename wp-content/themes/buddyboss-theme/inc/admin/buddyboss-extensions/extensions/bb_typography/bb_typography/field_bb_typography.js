...
(function( $ ) {
    ...
    redux.field_objects.bb_typography.init = function( selector, skipCheck ) {
                    ...
                    function() {
                        ...
                        $( this ).find( '.redux-typography-container' ).each(
                            function() {
                                ...
                                redux.field_objects.bb_typography.select( family, true );
                        ...
                        $( this ).find( '.redux-typography' ).on(
                            'change', function() {
                                redux.field_objects.bb_typography.select( $( this ) ); //.parents('.redux-container-typography:first'));
                            }
                        );
                        ...
                        $( this ).find(
                            ...
                            function() {
                                redux.field_objects.bb_typography.select(
                                    $( this ).parents( '.redux-container-bb_typography:first' )
                                );
                            }
                        );
                        ...
                        $( this ).find( '.redux-typography-color' ).wpColorPicker(
                            {
                                change: function( e, ui ) {
                                    ...
                                    redux.field_objects.bb_typography.select( $( this ) );
                                }
                            }
                        );
                        ...
                        $( this ).find( ".redux-typography-family" ).select2(
                            ...
                            "select2-opening", function( e ) {
                                ...
                                // Get field ID
                                var thisID = $( this ).parents( '.redux-container-bb_typography:first' ).attr( 'data-id' );
                                ...
                                if ( isUserFonts == 1 ) {
                                    ...
                                    // <option>
                                    for ( var key in redux.bb_typography[thisID] ) {
                                        var obj = redux.bb_typography[thisID].std_font;
    ...
    redux.field_objects.bb_typography.select = function( selector, skipCheck ) {
        var mainID;
        ...
        var google;
        if ( isSelecting === true ) {
            google = redux.field_objects.bb_typography.makeBool( selVals.object['data-google'] );
            $( '#' + mainID + ' .redux-typography-google-font' ).val( google );
        } else {
            google = redux.field_objects.bb_typography.makeBool(
            ...
            if ( google === true ) {
                ...
                $.each(
                    details.variants, function( index, variant ) {
                        if ( variant.id === style || redux.field_objects.bb_typography.size( details.variants ) === 1 ) {
                ...
                $.each(
                    details.subsets, function( index, subset ) {
                        if ( subset.id === script || redux.field_objects.bb_typography.size( details.subsets ) === 1 ) {
                            selected = ' selected="selected"';
                            script = subset.id;
                            $( '#' + mainID + ' input.typography-subsets' ).val( script );
                        } else {
        ...
        if ( $( '#' + mainID ).hasClass( 'typography-initialized' ) ) {
            ...
            var font_weight = 0;
            if ( style ) {
                font_weight = parseInt( style.match( /\d+/)[0], 10 );
            }

            $( '#' + mainID + ' .typography-preview' ).css( 'font-weight', font_weight );
            ...
            if ( color ) {
                ...
                $( '#' + mainID + ' .typography-preview' ).css(
                    'background-color', redux.field_objects.bb_typography.contrastColour( color )
                );
            }
