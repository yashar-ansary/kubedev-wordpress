		...
		class ReduxFramework_repeater {
			...
			public function render() {
				...
				if ( isset ( $this->value['redux_repeater_data'] ) && is_array( $this->value['redux_repeater_data'] ) && ! empty ( $this->value['redux_repeater_data'] ) ) {
					...
					foreach ( $repeaters as $repeater ) {
						...
						if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
							echo '<a href="javascript:void(0);" class="button deletion redux-repeaters-remove">' . __( 'Delete', 'buddyboss-theme' ) . ' ' . $this->field['item_name'] . '</a>';
						}
						echo '</div>';
						echo '</fieldset>';
						echo '</table>';
				...
				if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
					...
					echo '<a href="javascript:void(0);" class="button redux-repeaters-add button-primary' . $disabled . '" rel-id="' . $this->field['id'] . '-ul" rel-name="' . $this->parent->args['opt_name'] . $this->repeater_values . '[title][]">' . __( 'Add', 'buddyboss-theme' ) . ' ' . $this->field['item_name'] . '</a><br/>';
				}
			...
			public function enqueue() {
				...
				wp_enqueue_script(
					...
					apply_filters( "redux/repeater/{$this->parent->args['opt_name']}/enqueue/redux-field-repeater-js", $this->extension_url . 'field_repeater.js' ),
			...
			public function output_field( $field, $x ) {
				...
				if ( ! empty( $field['title'] ) ) {
					echo '<h4 class="bb-repeater-title">' . $field['title'] . '</h4>';
				}
			...
			function localize( $field, $value = array() ) {
				...
				// make sure $value is an array, so count() below works on PHP 7.2+.
				$value = (array) $value;
