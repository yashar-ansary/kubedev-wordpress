            ...
            <?php if ( ! empty( $settings['switch_search'] ) && 'yes' === $settings['switch_search'] ) : ?>
                <div id="courses-dir-search" class="bs-dir-search" role="search">
                    <div id="search-members-form" class="bs-search-form">
                        ...
                        <input type="text" name="search" id="bs_members_search" value="<?php echo ! empty( $_GET['search'] ) ? $_GET['search'] : ''; ?>"  placeholder="<?php _e( 'Search Courses...', 'buddyboss-theme' ); ?>">
                    </div>
                </div>
            <?php endif; ?>
        </div>
        <?php if ( ! empty( $settings['switch_courses_nav'] ) && 'yes' === $settings['switch_courses_nav'] ) : ?>
            <nav class="courses-type-navs main-navs bp-navs dir-navs bp-subnavs">
                <ul class="component-navigation courses-nav">
                    <?php
                    $navs = array(
                        'all' => __( 'All Courses', 'buddyboss-theme' ) . ' ' . '<span class="count">' . $query->found_posts . '</span>',
                    );

                    if ( is_user_logged_in() ) {
	                    $navs['my-courses'] = __( 'My Courses', 'buddyboss-theme' ) . '<span class="count">' . buddyboss_theme()->lifterlms_helper()->get_my_courses_count( null, $tax_query ) . '</span>';
                    }
        ...
        <div class="ld-secondary-header ld-secondary-header--llms">
            <div class="bb-secondary-list-tabs flex align-items-center" id="subnav"
                ...
                <div class="grid-filters <?php echo !empty($settings['grid_filter']) && 'on' === $settings['grid_filter'] ? 'active' : 'hide'; ?>" data-view="llms-course">
                    <a href="#" class="layout-view layout-view-course layout-grid-view bp-tooltip <?php echo !empty($class_grid_active) ? esc_attr( $class_grid_active ) : ''; ?>" data-view="grid" data-bp-tooltip-pos="up" data-bp-tooltip="<?php _e( 'Grid View', 'buddyboss-theme' ); ?>"><i class="dashicons dashicons-screenoptions" aria-hidden="true"></i>
                    </a>

                    <a href="#" class="layout-view layout-view-course layout-list-view bp-tooltip <?php echo !empty($class_list_active) ? esc_attr( $class_list_active ) : ''; ?>" data-view="list" data-bp-tooltip-pos="up" data-bp-tooltip="<?php _e( 'List View', 'buddyboss-theme' ); ?>"><i class="dashicons dashicons-menu" aria-hidden="true"></i>
                    </a>
                </div>
